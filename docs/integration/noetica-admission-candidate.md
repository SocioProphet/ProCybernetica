...
| Field                  | Value                                           |
|------------------------|-------------------------------------------------|
| `candidate.cluster_member_id`         | `noetica.chat.m2a`                              |
...
These must all pass before `candidate.status` may advance to `proposed`:
...
This plan must be tested and hash-bound before `candidate.status` advances to `admitted`.
...
Expected result: `PASS` with `candidate.status: proposed`.
