...
The Triune lab execution layer sits within a four-layer admission stack. Each layer has its own gate, its own evidence record, and its own `evidenceRef` / `pack_id`. Gates compose: a failure at any layer prevents admission at the layer above.
...
| `admission-pack.pack_id`                        | `PolicyAdmission.decisionRef`                 | `clusterAdmissionRef`                          |
...
3. The Triune `admission-pack.pack_id` is carried as `PolicyAdmission.decisionRef`.
...
1. The `ModelCatalogEntry.clusterAdmissionRef` must point to the Triune `pack_id` for that node.
