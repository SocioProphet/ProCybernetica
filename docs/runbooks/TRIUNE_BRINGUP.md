...
```text
Use a pinned, mirrored, verified, air-gapped k3s installer bundle.
Do not pipe live internet installer output into a shell from this runbook.
Disable bundled ingress, service load balancer, flannel, and default network policy so Cilium owns CNI and policy enforcement.
Record the installer digest, k3s version, and VM identity in the cluster-member record.
```
...
Run lane-specific structural validation with:
